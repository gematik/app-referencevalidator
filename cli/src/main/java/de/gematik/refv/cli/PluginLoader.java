...
public class PluginLoader {
  ...
  public Map<String, Plugin> loadPlugins(String pluginPath)
    ...
    for (var zipFile : zipFiles) {
      ...
      if (plugins.containsKey(plugin.getId())) {
        log.warn(
            "Duplicate plugin id found: '{}'. Change the id of the plugin in the plugin configuration file and try again",
            plugin.getId());
      } else plugins.put(plugin.getId(), plugin);
    }
    return plugins;
  }
