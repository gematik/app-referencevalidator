...
public class ModuleConfigurationPrinter {
  ...
  private static void addFHIRPackages(
      ValidationModuleConfiguration configuration, StringBuilder sb) {
    sb.append("\r\nFHIR-Package lists: \r\n");
    for (var dependencyListsEntrySet : configuration.getDependencyLists().entrySet()) {
      sb.append("  * " + dependencyListsEntrySet.getKey());
      if (dependencyListsEntrySet.getValue().getValidFrom() != null
          || dependencyListsEntrySet.getValue().getValidTill() != null) {
        sb.append("  (Valid ");
        if (dependencyListsEntrySet.getValue().getValidFrom() != null)
  ...
  private static void addSupportedProfiles(
      ValidationModuleConfiguration configuration, StringBuilder sb) {
    sb.append("\r\nSupported profiles:");
    for (var profileEntrySet : configuration.getSupportedProfiles().entrySet()) {
      sb.append("\r\n  * " + profileEntrySet.getKey());
      for (var versionsEntrySet : profileEntrySet.getValue().getProfileVersions().entrySet()) {
