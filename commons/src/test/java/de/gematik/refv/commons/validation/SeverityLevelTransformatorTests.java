...
class SeverityLevelTransformatorTests {
  ...
  void testTransformations() {
    ...
    var t2 =
        new ValidationMessageTransformation(
            "warning", "information", "Warning mes", "testMessageId", "testLocationString");
    ...
    Assertions.assertTrue(
        transformedMessages.stream()
            .allMatch(m -> m.getSeverity().equals(ResultSeverityEnum.INFORMATION)));
  }
  ...
  void testTransformationWithNonMatchingLocatorStringDoesntApply() {
    ...
    var notApplyingTransformation =
        new ValidationMessageTransformation(
            "error", "information", "not applying", "testMessageId", null);
    ...
    Assertions.assertTrue(
        transformedMessages.stream()
            .allMatch(m -> m.getSeverity().equals(ResultSeverityEnum.ERROR)));
  }
  ...
  void testTransformationWithMessageIdAndWithoutLocatorStringApply() {
    ...
    var notApplyingTransformation =
        new ValidationMessageTransformation("error", "information", null, "testMessageId", null);
    var transformations = List.of(notApplyingTransformation);
  ...
  void testTransformationWithNonMatchingMessageLocationRegexDoesntApply() {
    ...
    var notApplyingTransformation =
        new ValidationMessageTransformation(
            "error", "information", "Error message", "testMessageId", "not matching location");
    var transformations = List.of(notApplyingTransformation);
  ...
  void testTransformationIsNotPerformedOnNonMatchingMessageId() {
    ...
    var t1 =
        new ValidationMessageTransformation(
            "error", "information", "Error mes", "testMessageId", null);
    var transformations = List.of(t1);
  ...
  void testTransformationIsNotPerformedOnMatchingLocatorStringButNonMatchingMessageId() {
    ...
    var t1 =
        new ValidationMessageTransformation(
            "error", "information", "Error message", "someOtherMessageId", null);
    var transformations = List.of(t1);
  ...
  void testUndefinedValueSetsEscalatedToErrors() {
    ...
    m1.setMessage(
        "ValueSet https://fhir.kbv.de/ValueSet/KBV_VS_SFHIR_KBV_STATUSKENNZEICHEN not found by validator");
  ...
  void testUndefinedButIgnoredValueSetsAreNotEscalated() {
    ...
    var transformedMessages =
        engine.applyTransformations(inputMessages, new LinkedList<>(), List.of(ignoredValueSetUrl));
    Assertions.assertEquals(inputMessages.size(), transformedMessages.size());
    Assertions.assertTrue(
        transformedMessages.stream()
            .allMatch(m -> m.getSeverity().equals(ResultSeverityEnum.WARNING)),
        "Undefined but ignored value sets still produce error message");
  }
