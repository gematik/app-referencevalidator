  ...
  void testLatestDependencyListIsAppliedIfValidationOfTimePeriodIsDisabled() {
    ...
    var warningExists =
        result.getValidationMessages().stream()
            .anyMatch(
                m ->
                    m.getSeverity().equals(ResultSeverityEnum.WARNING)
                        && m.getMessageId()
                            .equals(GenericValidator.WARN_REFV_VALIDITY_PERIOD_CHECK_DISABLED));
    Assertions.assertTrue(
        warningExists, "No warning has been issued about deactivated validity period check");
  }
}
