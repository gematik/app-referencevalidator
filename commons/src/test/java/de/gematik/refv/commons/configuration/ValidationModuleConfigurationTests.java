...
class ValidationModuleConfigurationTests {
  ...
  void testFindFirstSupportedProfile() {
    ...
    supportedProfiles.put(
        "https://supported-profile1",
        new SupportedProfileVersions(
            new HashMap<>() {
              {
    ...
    var configuration =
        ValidationModuleConfiguration.builder().supportedProfiles(supportedProfiles).build();

    List<String> referencedProfiles =
        List.of(
            "http://unknown-profile1",
            "http://unknown-profile2",
            "https://supported-profile1|1.0.0",
            "https://supported-profile2|1.0.0");
    Profile firstSupportedProfile =
        configuration.findFirstSupportedProfileWithExistingConfiguration(referencedProfiles);
  ...
  @Test
  void
      testFindFirstSupportedProfileReturnsNotNullIfNoProfileVersionIsGivenAndTheProfileIsSupported() {
    var supportedProfiles = new HashMap<String, SupportedProfileVersions>();
    supportedProfiles.put(
        "https://supported-profile1",
        new SupportedProfileVersions(
  ...
  void testFindFirstSupportedProfileWhenNoSupportedProfilesPresent() {
    ...
    List<String> referencedProfiles =
        List.of("http://unknown-profile1", "http://unknown-profile2", "http://unknown-profile3");
    Profile firstSupportedProfile =
        configuration.findFirstSupportedProfileWithExistingConfiguration(referencedProfiles);
    assertThat(firstSupportedProfile).isNull();
  }
