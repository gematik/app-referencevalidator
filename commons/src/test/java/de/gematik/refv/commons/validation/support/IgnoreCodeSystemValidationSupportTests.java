...
class IgnoreCodeSystemValidationSupportTests {
  ...
  void testConfiguredCodeSystemsToIgnoreDoNotProduceErrors() {
    ...
    ValidationSupportChain validationSupportChain =
        new ValidationSupportChain(new DefaultProfileValidationSupport(fhirContext));
    ...
    Assertions.assertNull(
        result.getCode(),
        "result.getCode() should be empty, otherwise HAPI doesn't produce any ValidationMessages");
    Assertions.assertEquals(
        IValidationSupport.IssueSeverity.INFORMATION,
        result.getSeverity(),
        "Wrong severity of the code validation result");

    IValidationSupport.LookupCodeResult lookupCodeResult =
        support.lookupCode(context, systemA, "a-code", "en-US");
    Assertions.assertNotNull(
        lookupCodeResult, "Code lookup for an ignored code system is null while it should not be");
    Assertions.assertTrue(
        lookupCodeResult.isFound(),
        "Code lookup for an ignored code system is unsuccessful while it should be");
  }
  ...
  void testUnregisteredCodesystemsAreUnsupported() {
    ...
    Assertions.assertNull(
        support.lookupCode(context, systemC, "c-code", "en-US"),
        "Code lookup for a code of a not registered system is successful while it shouldn't be");
  }
