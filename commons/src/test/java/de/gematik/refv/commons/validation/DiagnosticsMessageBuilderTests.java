...
class DiagnosticsMessageBuilderTests {
  ...
  void testCreateValidityPeriodDiagnosticsString() {
    DependencyList dependencyList =
        new DependencyList("1999-01-01", "2000-01-01", new ArrayList<>(), new ArrayList<>());
    DependencyListsWrapper dependencyListsWrapper = new DependencyListsWrapper(dependencyList);
    String diagnostics =
        diagnosticsMessageBuilder.createValidityPeriodDiagnosticsString(
            dependencyListsWrapper, profileInResource, localDate);
    Assertions.assertEquals(
        "Profile http://profile/in/resource is invalid for the creation date of the resource (1999-01-01); only valid from 1999-01-01 till 2000-01-01",
        diagnostics);
  }
  ...
  void testCreateValidityPeriodDiagnosticsStringValidityPeriodOptionalIsEmpty() {
    ...
    String diagnostics =
        diagnosticsMessageBuilder.createValidityPeriodDiagnosticsString(
            dependencyListsWrapper, profileInResource, localDate);
    Assertions.assertEquals(
        "Profile http://profile/in/resource is invalid for the creation date of the resource (1999-01-01)",
        diagnostics);
  }
  ...
  void testCreateValidityPeriodDiagnosticsStringValidFromIsEmptyThrowsIllegalStateException() {
    DependencyList dependencyList =
        new DependencyList(null, "2000-01-01", new ArrayList<>(), new ArrayList<>());
    DependencyListsWrapper dependencyListsWrapper = new DependencyListsWrapper(dependencyList);
  ...
  void testCreateValidityPeriodDiagnosticsStringValidTillIsEmpty() {
    DependencyList dependencyList =
        new DependencyList("1999-01-01", null, new ArrayList<>(), new ArrayList<>());
    DependencyListsWrapper dependencyListsWrapper = new DependencyListsWrapper(dependencyList);
    String diagnostics =
        diagnosticsMessageBuilder.createValidityPeriodDiagnosticsString(
            dependencyListsWrapper, profileInResource, localDate);
    Assertions.assertEquals(
        "Profile http://profile/in/resource is invalid for the creation date of the resource (1999-01-01); only valid from 1999-01-01",
        diagnostics);
  }
}
