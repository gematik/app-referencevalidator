...
public class UcumValidationSupport implements IValidationSupport {
  ...
  public LookupCodeResult lookupCode(
    ...
    if (theSystem.equals(UCUM_CODESYSTEM_URL)) {
      ...
      try {
        ...
        log.debug(
            "Failed to parse UCUM code {}: {}",
            theCode,
            e.getMessage()); // Deviation from CommonCodeSystemsTerminologyService.lookupUcumCode
        // --> log.level <- DEBUG
      } finally {
  ...
  public CodeValidationResult validateCode(
    ...
    return new CodeValidationResult().setMessage("Validation failed").setSeverity(issueSeverity);
  }
}
