...
class SeverityLevelTransformer {
  ...
  public List<SingleValidationMessage> applyTransformations(
      ...
      @NonNull Collection<ValidationMessageTransformation> transformations) {
  ...
  public List<SingleValidationMessage> applyTransformations(
    ...
    for (SingleValidationMessage message : transformedMessages) {

      var transformation =
          transformations.stream()
              .filter(
                  t ->
                      t.getSeverityLevelFrom().equals(message.getSeverity().getCode())
                          && messageMatchesPattern(message.getMessage(), t.getLocatorString())
                          && (t.getMessageId() == null
                              || Strings.CS.equals(t.getMessageId(), message.getMessageId()))
                          && (t.getMessageLocationRegex() == null
                              || messageMatchesPattern(
                                  message.getLocationString(), t.getMessageLocationRegex())))
              .findFirst();
  ...
  private static boolean messageMatchesPattern(String message, String locator) {
    if (StringUtils.isEmpty(locator)) return true;
  ...
  private void setIgnoredCodeSystemsToInformation(LinkedList<SingleValidationMessage> messages) {
    for (SingleValidationMessage message : messages) {
      if (I18nConstants.TERMINOLOGY_PASSTHROUGH_TX_MESSAGE.equals(message.getMessageId())
          && message
              .getMessage()
              .contains(IgnoreCodeSystemValidationSupport.CODE_SYSTEM_IS_IGNORED_MESSAGE)) {
        message.setSeverity(ResultSeverityEnum.INFORMATION);
      }
    }
  }

  private void escalateUnresolvedValueSetsToError(
      LinkedList<SingleValidationMessage> messages, Collection<String> ignoredValueSets) {
    for (SingleValidationMessage message : messages) {
      if (I18nConstants.TERMINOLOGY_TX_VALUESET_NOTFOUND.equals(message.getMessageId())) {
        ...
        if (ignoredValueSets.stream()
            .noneMatch(ignoredValueSet -> message.getMessage().contains(ignoredValueSet)))
