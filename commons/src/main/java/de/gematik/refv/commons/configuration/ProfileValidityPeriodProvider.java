...
public class ProfileValidityPeriodProvider {
  ...
  public Optional<ProfileValidityPeriod> calculateFrom(DependencyListsWrapper dependencyLists) {
    if (dependencyLists.areNoValidityPeriodsDefined()) return Optional.empty();
  ...
  private static Optional<LocalDate> findMaximalValidityTill(
      DependencyListsWrapper dependencyLists) {
    if (dependencyLists.isAnyDependencyListWithOpenEndExists()) return Optional.empty();
    else return dependencyLists.getLatestValidTill();
  }

  private static Optional<LocalDate> findMinimalValidityFrom(
      DependencyListsWrapper dependencyLists) {
