...
class HapiFhirValidatorFactory {
  ...
  public FhirValidator createInstance(
    ...
    addNpmPackagesToValidationSupportChain(
        packageFilenames, validationSupportChain, validationModuleResourceProvider);
    return createHapiFhirValidator(configuration, validationSupportChain);
  }

  private FhirValidator createHapiFhirValidator(
  ...
  private void addNpmPackagesToValidationSupportChain(
      Collection<String> packageFilenames,
      ValidationSupportChain validationSupportChain,
      ValidationModuleResourceProvider validationModuleResourceProvider) {
    for (String packagePath : packageFilenames) {
      var prePopulatedValidationSupport =
          packageCache.addOrGet(packagePath, validationModuleResourceProvider);
