...
class DiagnosticsMessageBuilder {
  ...
  public String createValidityPeriodDiagnosticsString(
      DependencyListsWrapper dependencyLists,
      Profile profileInResource,
      LocalDate resourceCreationDate) {
    ...
    StringBuilder pattern =
        new StringBuilder("Profile %s is invalid for the creation date of the resource (%s)");
            ...
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        "Unexpected validity period list without start date detected for profile "
                            + profileInResource));
    ...
    return String.format(
        pattern.toString(), profileInResource, resourceCreationDate, validFrom, validTill);
  }
}
