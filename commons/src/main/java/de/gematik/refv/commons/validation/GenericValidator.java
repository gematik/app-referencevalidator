...
public class GenericValidator {
  public static final String ERR_REFV_PROFILE_OUTSIDE_OF_VALIDITY_PERIOD =
      "REFV_PROFILE_OUTSIDE_OF_VALIDITY_PERIOD";
  public static final String ERR_REFV_NO_CREATION_DATE_IN_RESOURCE =
      "REFV_NO_CREATION_DATE_IN_RESOURCE";
  public static final String ERR_REFV_PARSE_ERROR = "REFV_PARSE_ERROR";
  public static final String WARN_REFV_VALIDITY_PERIOD_CHECK_DISABLED =
      "REFV_VALIDITY_PERIOD_CHECK_DISABLED";
  public static final String WARN_REFV_PASSED_PROFILE_DIFFERS_FROM_META_PROFILE =
      "REFV_WARN_PASSED_PROFILE_DIFFERS_FROM_META_PROFILE";
  ...
  public GenericValidator(FhirContext context) {
    ...
    // Do not evict internal cache of FhirInstanceValidator as the object itself is kept in memory
    // and its cache is permanently required
    // Mediates the memory leak issue https://github.com/hapifhir/org.hl7.fhir.core/issues/1412
    // (occurs in HAPI 6.6.2)
    System.setProperty(
        "TEST_SYSTEM_PROP_VALIDATION_RESOURCE_CACHES_MS", String.valueOf(Long.MAX_VALUE));
  }

  public ValidationResult validate(
      @NonNull String resourceBody, @NonNull ValidationModuleResourceProvider resourceProvider)
      throws IllegalArgumentException {
    return validate(resourceBody, resourceProvider, ValidationOptions.getDefaults());
  }

  public ValidationResult validate(
      @NonNull String resourceBody,
    ...
    if (!validateEncoding(resourceBody, configuration, validationOptions))
      ...
      if (validationOptions.getProfileFilterRegex() != null
          && noneReferencedProfileMatches(
              allReferencedProfilesInResource, validationOptions.getProfileFilterRegex())) {
        result =
            ValidationResult.createInstance(
                ResultSeverityEnum.ERROR,
                ERR_REFV_PROFILE_FILTER_MISMATCH,
                String.format(
                    "None of the referenced profiles in the resource matches the profile filter: %s. Referenced profiles: %s",
                    validationOptions.getProfileFilterRegex(), allReferencedProfilesInResource));
      } else {
  ...
  private static boolean noneReferencedProfileMatches(
      List<String> allReferencedProfilesInResource, Pattern profileFilterRegex) {
    log.debug(
        "Checking if any of the referenced profiles in the resource matches the profile filter: {}...",
        profileFilterRegex);
    return allReferencedProfilesInResource.stream()
        .noneMatch(p -> profileFilterRegex.matcher(p).find());
  }

  private ValidationResult validateResource(
      ResourceToValidate resource,
      ValidationModuleResourceProvider resourceProvider,
      ProfileValidityPeriodCheckStrategy profileValidityPeriodCheckStrategy,
      Profile profileForValidation) {
    var configuration = resourceProvider.getConfiguration();
    ...
    if (StringUtils.isEmpty(creationDateLocator)) {
      log.debug(
          "No creation date locator expression is configured for the profile {}. Validating against all configured dependency lists...",
          profileForValidation);
      return validateUsingMultipleDependencyLists(
          resource,
          resourceProvider,
          profileForValidation,
          dependencyListsWrapper.getDependencyLists());
    } else {
      return validateUsingConfiguredLocator(
          resource,
          resourceProvider,
          profileValidityPeriodCheckStrategy,
          creationDateLocator,
          dependencyListsWrapper,
          profileForValidation);
    }
  }

  private ValidationResult validateUsingConfiguredLocator(
      ResourceToValidate resource,
      ValidationModuleResourceProvider resourceProvider,
      ProfileValidityPeriodCheckStrategy profileValidityPeriodCheckStrategy,
      String creationDateLocator,
      DependencyListsWrapper dependencyListsWrapper,
      Profile profileForValidation) {
    try {
      var resourceCreationDateOptional =
          new ResourceCreationDateLocator(fhirContext)
              .findCreationDateIn(resource.getBody(), creationDateLocator);
      ...
      if (resourceCreationDateOptional.isEmpty()) {
        ...
        return ValidationResult.createInstance(
            ResultSeverityEnum.ERROR,
            ERR_REFV_NO_CREATION_DATE_IN_RESOURCE,
            String.format(
                "Could not determine the creation date of the resource using the configured expression %s",
                creationDateLocator));
      }

      // Use Case 2.2 Creation date could be located in the resource
      List<DependencyList> dependencyLists =
          dependencyListsWrapper.getDependencyListsValidAt(resourceCreationDateOptional.get());
      if (!dependencyLists.isEmpty()) {
        // Use Case 2.2.1 At least one dependency list for the resource creation date is present
        return validateUsingMultipleDependencyLists(
            resource, resourceProvider, profileForValidation, dependencyLists);
      }
      ...
      if (profileValidityPeriodCheckStrategy == ProfileValidityPeriodCheckStrategy.VALIDATE) {
        // Use Case 2.2.2.1 ValidityPeriodCheck is turned on and no dependency lists were found ->
        // Profile is outside of validity period!
        String diagnostics =
            new DiagnosticsMessageBuilder()
                .createValidityPeriodDiagnosticsString(
                    dependencyListsWrapper,
                    profileForValidation,
                    resourceCreationDateOptional.get());
        return ValidationResult.createInstance(
            ResultSeverityEnum.ERROR, ERR_REFV_PROFILE_OUTSIDE_OF_VALIDITY_PERIOD, diagnostics);
      }

      // Use Case 2.2.2.2 ValidityPeriodChek is turned off -> use latest dependencies
      var result =
          validateUsingDependencyList(
              resource,
              resourceProvider,
              dependencyListsWrapper.getLatestDependencyList(),
              profileForValidation);
      ...
      return ValidationResult.createInstance(
          ResultSeverityEnum.ERROR, ERR_REFV_PARSE_ERROR, e.getMessage());
    }
  }

  private ValidationResult validateUsingMultipleDependencyLists(
      ResourceToValidate resource,
      ValidationModuleResourceProvider resourceProvider,
      Profile profileForValidation,
      List<DependencyList> dependencyLists) {
    ...
    for (var dependencyList : dependencyLists) {
      result =
          validateUsingDependencyList(
              resource, resourceProvider, dependencyList, profileForValidation);
  ...
  private static List<String> getAcceptedEncodings(
      ValidationModuleConfiguration configuration, ValidationOptions validationOptions) {
    var encodings =
        !validationOptions.getAcceptedEncodings().isEmpty()
            ? validationOptions.getAcceptedEncodings()
            : configuration.getAcceptedEncodings();
  ...
  private ValidationResult validateUsingDependencyList(
      ResourceToValidate resource,
      ValidationModuleResourceProvider resourceProvider,
    ...
    var fhirValidator =
        hapiFhirValidatorCache.computeIfAbsent(
            dependencyList,
            k ->
                hapiFhirValidatorFactory.createInstance(
                    dependencyList.getPackages(), resourceProvider));

    var options = new ca.uhn.fhir.validation.ValidationOptions();
    boolean isValidationProfileReferencedInTheResource =
        resource.getProfiles().contains(profileForValidation.getCanonical());
    ...
    log.debug(
        "Pre-Transformation ValidationResult: Valid: {}, Messages: {}",
        intermediateResult.isSuccessful(),
        intermediateResult.getMessages());

    var filteredMessages =
        severityLevelTransformator.applyTransformations(
            intermediateResult.getMessages(),
            dependencyList.getValidationMessageTransformations(),
            ignoredValueSets);
    ...
    if (!isValidationProfileReferencedInTheResource) {
      ...
      m.setMessage(
          String.format(
              "Resource meta.profile differs from the passed profile for validation: meta.profile=%s; passed=%s",
              resource.getProfiles(), profileForValidation));
