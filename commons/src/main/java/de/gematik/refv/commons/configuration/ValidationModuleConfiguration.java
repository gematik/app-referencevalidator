...
public class ValidationModuleConfiguration {
  ...
  @Builder.Default
  private IValidationSupport.IssueSeverity ucumValidationSeverityLevel =
      IValidationSupport.IssueSeverity.ERROR;

  public Map<String, DependencyList> getDependencyLists() {
    return Collections.unmodifiableMap(
        Objects.requireNonNullElse(dependencyLists, new HashMap<>()));
  }

  public Map<String, SupportedProfileVersions> getSupportedProfiles() {
    return Collections.unmodifiableMap(
        Objects.requireNonNullElse(supportedProfiles, new HashMap<>()));
  }

  public List<String> getIgnoredCodeSystems() {
    return Collections.unmodifiableList(
        Objects.requireNonNullElse(ignoredCodeSystems, new LinkedList<>()));
  }

  public List<String> getIgnoredValueSets() {
    return Collections.unmodifiableList(
        Objects.requireNonNullElse(ignoredValueSets, new LinkedList<>()));
  }

  public List<String> getAcceptedEncodings() {
    return Collections.unmodifiableList(
        Objects.requireNonNullElse(acceptedEncodings, new LinkedList<>()));
  }
  ...
  public Optional<ProfileConfiguration> getSupportedProfileConfiguration(Profile profile) {
    ...
    if (getSupportedProfiles().isEmpty()) {
      ...
      // But if we don't return a dependencyList here,the validator will always return an
      ...
      if (firstDependencyList.isEmpty())
        throw new IllegalStateException(
            "Supported profiles are not explicitly defined and no dependency lists found. Define at least one dependency list");
  ...
  public DependencyListsWrapper getDependencyListsForProfile(
    ...
    if (!allKeysExist)
      throw new IllegalStateException(
          String.format(
              "Some of the dependency lists are undefined: %s",
              String.join(",", profileConfiguration.getDependencyLists())));

    return new DependencyListsWrapper(
        lists.stream().map(key -> getDependencyLists().get(key)).collect(Collectors.toList()));
  }

  public Profile findFirstSupportedProfileWithExistingConfiguration(List<String> profiles) {
    List<Profile> supportedProfilesFound =
        profiles.stream()
            .map(Profile::parse)
