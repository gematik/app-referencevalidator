...
public class ReferencedProfileLocator {
  ...
  private List<String> locateProfileInMetaTagXml(XMLEventReader xmlEventReader)
      ...
      if (event.isStartElement()) {
        ...
        if (nextTag.getName().getLocalPart().equalsIgnoreCase(PROFILE_STRING)) {
          Attribute valueAttribute =
              nextTag.asStartElement().getAttributeByName(new QName("", "value", ""));
  ...
  private List<String> getAllProfilesInXml(XMLEventReader xmlEventReader, Attribute valueAttribute)
      throws XMLStreamException {
  ...
  private void parseXmlEventsForProfiles(XMLEventReader xmlEventReader, List<String> profilesFound)
      throws XMLStreamException {
    XMLEvent event;
    StartElement nextTag;
    while (xmlEventReader.hasNext()) {
  ...
  private List<String> getAllProfilesInJson(JsonParser jsonParser) throws IOException {
    ...
    while (!"]".equals(jsonParser.getText()) && !"}".equals(jsonParser.getText())) {
      ...
      if (!StringUtils.isEmpty(profileUrlWithoutVersion) && !"}".equals(profileUrlWithoutVersion)) {
