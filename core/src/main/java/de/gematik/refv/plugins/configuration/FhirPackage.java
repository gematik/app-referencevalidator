...
public class FhirPackage {
  ...
  private static final String MALFORMED_PACKAGE_DECLARATION_TEXT =
      "Malformed package name '%s' (expected format: packageName#packageVersion)";
  ...
  public FhirPackage(String fhirPackageNameAndVersionWithRouteSeparator) {
    String[] validationFhirPackageNameAndVersion =
        fhirPackageNameAndVersionWithRouteSeparator.split("#");
    if (validationFhirPackageNameAndVersion.length < 2) {
      throw new MalformedPackageDeclarationException(
          String.format(
              MALFORMED_PACKAGE_DECLARATION_TEXT, fhirPackageNameAndVersionWithRouteSeparator));
    }
