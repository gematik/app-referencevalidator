...
public class ValidationModuleConfigurationFactory {
  ...
  private static void addNewSupportedProfilesToMap(
      String profile,
      Map<String, ProfileConfiguration> newProfileVersions,
      Map<String, Map<String, ProfileConfiguration>> supportedProfiles) {
    supportedProfiles.compute(
        profile,
  ...
  private static void mergeProfileVersions(
      Map<String, ProfileConfiguration> existingProfileVersions,
      Map<String, ProfileConfiguration> newProfileVersions,
      String profile) {
    for (var newProfileVersion : newProfileVersions.entrySet()) {
      ...
      var combinedDependencyList =
          new ArrayList<>(existingProfileConfiguration.getDependencyLists());
      ...
      if (!Objects.equals(
          newProfileVersion.getValue().getCreationDateLocator(),
          existingProfileConfiguration.getCreationDateLocator()))
        throw new IllegalArgumentException(
            "Multiple creation date locators defined for profile: "
                + profile
                + ". Configuration for version-less canonicals can not be created");
      // creationDateLocator is identical for all dependency lists and can be overridden
      existingProfileVersions.put(
          newProfileVersion.getKey(),
          new ProfileConfiguration(
              combinedDependencyList, newProfileVersion.getValue().getCreationDateLocator()));
    }
  }
  ...
  private static DependencyList createDependencyListSectionPart(
    ...
    if (dl.getDependencies() != null) {
      dependencies.addAll(
          dl.getDependencies().stream().map(FhirPackage::toFilename).collect(Collectors.toList()));
    }
