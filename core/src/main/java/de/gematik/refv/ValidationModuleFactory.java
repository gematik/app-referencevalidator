...
public class ValidationModuleFactory {
  ...
  public ValidationModule createValidationModuleFromPlugin(@NonNull Plugin plugin)
    ...
    ValidationModule result =
        new BaseValidationModule(
            new PluginValidationResourceProvider(plugin::getResource, fhirContext), engine);
    result.initialize();
    return result;
  }

  public ValidationModule createValidationModule(@NonNull SupportedValidationModule module)
      throws ValidationModuleInitializationException {
    ValidationModule result;
    if (module == SupportedValidationModule.ERPTA7) {
      result = new ErpTa7ValidationModule(new IntegratedValidationResourceProvider("erpta7"));
    } else {
