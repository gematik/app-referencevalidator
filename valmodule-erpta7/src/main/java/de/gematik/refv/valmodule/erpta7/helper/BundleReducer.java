...
public class BundleReducer {
  ...
  public List<String> getAllRezeptBundlesAsString()
    ...
    NodeList resources =
        (NodeList)
            xpath
                .compile("/Bundle/entry/resource/Bundle")
  ...
  public String getReducedResourceBody() throws XPathExpressionException, TransformerException {
    ...
    NodeList compositionNodes =
        (NodeList) compositionExpression.evaluate(entries.item(0), XPathConstants.NODESET);
    if (compositionNodes.getLength() == 0) {
      // First entry is not Composition --> keep it in the reduced body (which will produce
      // validation errors later if Bundle is of type document)
  ...
  private String removeDangledReferencesFromComposition(String reducedBodyAsString) {
    ...
    var compositionOptional =
        bundle.getEntry().stream().filter(e -> e.getResource().hasType("Composition")).findFirst();
    ...
    var sectionsWithRechnungBundles =
        c.getSection().stream()
            .filter(BundleReducer::isSectionWithReferenceToRechnungBundle)
            .collect(Collectors.toList());
    sectionsWithRechnungBundles.stream()
        .filter(s -> isSectionReferenceMissingInReducedBundle(s, allFullUrls))
        .forEach(s -> c.getSection().remove(s));

    return xmlParser.encodeResourceToString(bundle);
  }

  private static boolean isSectionReferenceMissingInReducedBundle(
      Composition.SectionComponent s, List<String> allFullUrls) {
  ...
  private void copyFirstRezeptBundle(Document newDoc, Element rootElement, NodeList entries)
    ...
    for (int i = 0; i < entries.getLength(); i++) {
      ...
      NodeList bundleNodes =
          (NodeList) bundleExpression.evaluate(entryNode, XPathConstants.NODESET);
  ...
  // This implementation produces different line endings in different environments (e.g. local vs
  // jenkins).
  // But this doesn't affect the validation process and is faster than other implementations that
  // set fix line endings (e.g. LSSerializer).
