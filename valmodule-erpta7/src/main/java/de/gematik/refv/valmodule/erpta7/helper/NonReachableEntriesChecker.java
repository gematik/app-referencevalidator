...
public class NonReachableEntriesChecker {
  ...
  private static final String RULE_BDL_7_MESSAGE =
      "Entry '%s' isn't reachable by traversing links (forward or backward) from the Composition";
  ...
  public List<SingleValidationMessage> checkForNonReachableEntries(Document resource) {
    if (!isDocumentType(resource)) return new LinkedList<>();
    ...
    Arrays.stream(allFullUrls)
        .filter(fullUrl -> Arrays.stream(referencedEntries).noneMatch(fullUrl::endsWith))
        .forEach(
            fullUrl -> {
  ...
  private String[] getEntriesReferencedInComposition(Document resource) {
    XPath xPath = xPathFactory.newXPath();
    XPathExpression exp =
        xPath.compile("/Bundle/entry/resource/Composition/section/entry/reference/@value");
  ...
  private String[] getAllFullUrls(Document resource) {
    XPath xPath = xPathFactory.newXPath();
    XPathExpression exp =
        xPath.compile("/Bundle/entry[not(self::entry/resource/Composition)]/fullUrl/@value");
