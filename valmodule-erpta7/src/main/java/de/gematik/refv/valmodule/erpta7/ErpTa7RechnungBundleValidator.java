...
public class ErpTa7RechnungBundleValidator {
  ...
  private final NonReachableEntriesChecker nonReachableEntriesChecker =
      new NonReachableEntriesChecker();
  ...
  @SneakyThrows
  public ValidationResult validateBundleConcurrently(
      String resourceBody, ValidationOptions options) {
    Document document = documentParser.getDocument(resourceBody);

    var nonReachableEntriesValidationMessages =
        nonReachableEntriesChecker.checkForNonReachableEntries(document);
    ...
    var reducedBodyValidationMessages =
        validator.validateString(reducedResourceBody, options).getValidationMessages();
    var concurrentValidationMessages =
        performConcurrentValidation(allRezeptBundlesAsString, options).stream()
            .map(ValidationResult::getValidationMessages);
  ...
  private Collection<ValidationResult> performConcurrentValidation(
      List<String> allEntriesAsStrings, ValidationOptions options) {
