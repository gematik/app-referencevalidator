...
class NonReachableEntriesCheckerTests {
  ...
  void testIfNonReachableEntriesAreFoundForDocuments() {
    var testBundle =
        getTestResourceBody(
            "non-reachable-entries-checker/document-bundle-with-non-reachable-entries.xml");
    ...
    Assertions.assertThat(messages.get(0).getMessage())
        .contains(
            "Entry 'E' isn't reachable by traversing links (forward or backward) from the Composition");
  }
  ...
  void testIfNonReachableEntriesAreIgnoredForCollections() {
    var testBundle =
        getTestResourceBody(
            "non-reachable-entries-checker/collection-bundle-with-non-reachable-entries.xml");
    var documentBuilder = DocumentBuilderFactory.newInstance().newDocumentBuilder();
    var xmlDoc = documentBuilder.parse(new InputSource(new StringReader(testBundle)));
    var messages = new NonReachableEntriesChecker().checkForNonReachableEntries(xmlDoc);
