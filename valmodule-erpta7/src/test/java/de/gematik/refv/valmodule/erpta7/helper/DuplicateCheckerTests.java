...
class DuplicateCheckerTests {
  ...
  void testFindDuplicateFullUrls() {
    ...
    validationMessage.setMessage(
        "'FullUrl must be unique in a bundle, or else entries with the same fullUrl must have different meta.versionId (except in history bundles)' Failed for fullUrl: "
            + "urn:uuid:dfdee8be-e6d3-42bb-ba30-a2a0addf8ee9");
    ...
    assertThat(messages).hasSize(1).contains(validationMessage);
  }
